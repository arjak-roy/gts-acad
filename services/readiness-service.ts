...
/**
 * Maps a numeric readiness score into a placement lifecycle status.
 * Applies the configured threshold and a review band below it.
 * Centralizes status rules so all readiness updates stay consistent.
 */
...
/**
 * Recomputes readiness for a learner using attendance and assessment signals.
 * Persists learner rollup fields and creates a readiness snapshot atomically.
 * Throws explicit errors when DB prerequisites or learner records are missing.
 */
...
/**
 * Synchronizes a placement-ready learner to an external recruiter destination.
 * Recomputes readiness first, then logs outbound payload and response metadata.
 * Updates learner and snapshot sync status in one transactional workflow.
 */

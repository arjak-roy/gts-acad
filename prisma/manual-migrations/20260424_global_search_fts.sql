-- =============================================================================
-- Global Search: Full-Text Search Infrastructure
-- =============================================================================
-- Creates pg_trgm extension, a materialized view unifying 12 searchable
-- entities, GIN indexes for tsvector and trigram similarity, and a helper
-- function to refresh the index.
-- =============================================================================

-- 1. Enable the trigram extension for fuzzy / similarity matching
CREATE EXTENSION IF NOT EXISTS pg_trgm;

-- 2. Drop existing objects if re-running
DROP MATERIALIZED VIEW IF EXISTS search_index;

-- 3. Create the unified search materialized view
...
SELECT
  ...
  'learning_resources',
  lr.title,
  COALESCE(lrc.name, '') || ' | ' || COALESCE(lr.content_type::text, '') || ' | ' || COALESCE(lr.visibility::text, ''),
...
-- 5. Populate the materialized view
REFRESH MATERIALIZED VIEW search_index;

-- 6. Helper function to refresh the materialized view concurrently
